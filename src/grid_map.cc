...
double GridMap::minX()
{
    return (0 - init_x_) * cell_size_;
}

double GridMap::maxX()
{
    return (size_x_ - init_x_) * cell_size_;
}

double GridMap::minY()
{
    return (0 - init_y_) * cell_size_;
}

double GridMap::maxY()
{
    return (size_y_ - init_y_) * cell_size_;
}
...
// TODO 
void GridMap::loadMap ( const std::string& img_dir, const std::string& cfg_dir )
{
    /* 利用opencv读取地图图片 */
    cv::Mat cv_map = cv::imread(img_dir);
...
void GridMap::loadMap ( const std::string& img_dir, const int& init_x, const int& init_y, const double& cell_size )
{
    /* 利用opencv读取地图图片 */
    cv::Mat cv_map = cv::imread(img_dir);
    cv::Mat cv_gray;
    cv::cvtColor(cv_map, cv_gray, cv::COLOR_RGB2GRAY);
    
    cv::flip(cv_gray, cv_gray, 1);
    
    size_x_ = cv_gray.cols;
    size_y_ = cv_gray.rows;
    init_x_ = init_x;
    init_y_ = init_y;
    ...
    for (size_t x=0; x<cv_gray.cols; x++)
    {
        for( size_t y=0; y<cv_gray.rows; y++)
        {
            uint8_t pixel = cv_gray.at<uint8_t>(y, x);   
            
            // TODO 这里的参数没有加入到配置文件中
            switch (pixel)
            {
                case 0: //占用
                    bel_data_(x, y) = 1.0;
